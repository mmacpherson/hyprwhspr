...
def _get_layout_from_hyprland() -> tuple[str, str]:
    ...
    try:
        result = subprocess.run(
            ["hyprctl", "devices", "-j"], capture_output=True, text=True, timeout=2
        )
...
class GlobalShortcuts:
    ...
    def __init__(
        ...
        grab_keys: bool = True,
        keyboard_device_names: Optional[List[str]] = None,
    ):
        ...
        self.keyboard_device_names: Optional[List[str]] = (
            [n.lower() for n in keyboard_device_names]
            if keyboard_device_names
            else None
        )
        ...
        self._device_lock = (
            threading.Lock()
        )  # Protect device list from concurrent modification
        ...
        self.debounce_time = (
            0.1  # 100ms debounce - shorter for push-to-talk responsiveness
        )
    ...
    def _discover_keyboards(self):
        ...
        with self._device_lock:
            ...
            try:
                ...
                # Never grab our own UInput virtual keyboard. Doing so creates a
                # feedback loop (physical key → virtual → re-grab) that locks
                ...
                if self.selected_device_name:
                    ...
                    try:
                        for device in devices:
                            try:
                                ...
                                if (
                                    device_name_lower == search_name_lower
                                    or search_name_lower == device_name_lower
                                ):
                        ...
                        if not matching_devices:
                            print(
                                f"[WARN] Selected device name '{self.selected_device_name}' not found!"
                            )
                            print(
                                "[WARN] Use 'hyprwhspr keyboard list' to see available devices"
                            )
                        ...
                        if len(matching_devices) > 1:
                            print(
                                f"[WARN] Multiple devices match '{self.selected_device_name}':"
                            )
                            for dev in matching_devices:
                                print(f"[WARN]   - {dev.name} ({dev.path})")
                            print(
                                f"[WARN] Using first match: {matching_devices[0].name} ({matching_devices[0].path})"
                            )
                    ...
                    if selected_device is None:
                        print(
                            f"[WARN] Selected device path {self.selected_device_path} not found!"
                        )
                        print(
                            "[WARN] Use 'hyprwhspr keyboard list' to see available devices"
                        )
                ...
                for device in devices:
                    ...
                    if ecodes.EV_KEY not in capabilities:
                        if self.selected_device_name or self.selected_device_path:
                            print(
                                f"[ERROR] Selected device '{device.name}' ({device.path}) does not support keyboard events (EV_KEY)"
                            )
                            print(
                                "[ERROR] This device cannot be used for keyboard shortcuts"
                            )
                            device.close()
                            return
                        device.close()
                    ...
                    if not self.target_keys.issubset(available_keys):
                        if self.selected_device_name or self.selected_device_path:
                            ...
                            missing_key_names = [
                                self._keycode_to_name(k) for k in missing_keys
                            ]
                            print(
                                f"[ERROR] Selected device '{device.name}' ({device.path}) cannot emit all keys required for shortcut '{self.primary_key}'"
                            )
                            print(
                                f"[ERROR] Missing keys: {', '.join(missing_key_names)}"
                            )
                            print(
                        ...
                        for retry in range(2):
                            try:
                                ...
                                if (
                                    ...
                                    or self.selected_device_path
                                ):
                                    print(
                                        f"[ERROR] Cannot access selected device '{device.name}' ({device.path}): {e}"
                                    )
                                    print(
                                        "[ERROR] This usually means you need root or input group membership"
                                    )
                                    print(
                                ...
                                print(
                                    "[WARN]   This usually means you need root or input group membership"
                                )
                                device.close()
                                break
            ...
            if not self.devices:
                if self.selected_device_name:
                    ...
                    print(
                        f"[ERROR] Selected device name '{self.selected_device_name}' could not be initialized"
                    )
                    print(
                        "[ERROR] Use 'hyprwhspr keyboard list' to see available devices"
                    )
                elif self.selected_device_path:
                    # This shouldn't happen if we handled all cases above, but just in case
                    print(
                        f"[ERROR] Selected device path {self.selected_device_path} could not be initialized"
                    )
                    print(
                        "[ERROR] Use 'hyprwhspr keyboard list' to see available devices"
                    )
                    ...
                    print(
                        "[ERROR] No accessible devices found that can emit the configured shortcut!"
                    )
                    print("[ERROR] Solutions:")
                    print(
                        "[ERROR]   1. Add yourself to 'input' group: sudo usermod -aG input $USER (then log out and back in)"
                    )
                    if self.grab_keys:
                        print(
                            "[ERROR]   2. Disable key grabbing in config (grab_keys: false)"
                        )
                        print(
                            f"[ERROR]   3. Check that your shortcut '{self.primary_key}' uses keys available on your keyboard"
                        )
                    else:
                        print(
                            "[ERROR]   2. Or use compositor bindings: set use_hypr_bindings: true and bind the shortcut in Hyprland to write to recording_control (see docs)"
                        )
                        print(
                            f"[ERROR]   3. Check that your shortcut '{self.primary_key}' uses keys available on your keyboard"
                        )
    ...
    def _parse_key_combination(self, key_string: str) -> Set[int]:
        ...
        if not keys:
            print(
                f"Warning: Could not parse key combination '{key_string}', defaulting to F12"
            )
    ...
    def _string_to_keycode(self, key_string: str) -> Optional[int]:
        ...
        if code is None:
            print(
                f"Warning: Unknown key string '{original}' (resolved to '{key_name}')"
            )
            return None
                ...
                for fd in ready_fds:
                    if fd in device_fds_snapshot:
                        ...
                        try:
                            ...
                            print(
                                f"[ERROR] Lost connection to device: {device.name}: {e}"
                            )
    ...
    def _process_event(self, event):
        ...
        if event.type == ecodes.EV_KEY:
            ...
            if key_event.keystate == key_event.key_down:
                ...
                if event.code in self.target_keys and self.target_keys.issubset(
                    self.pressed_keys
                ):
                    extra_modifiers = (
                        self.pressed_keys - self.target_keys
                    ) & self.MODIFIER_KEYS
                    if (
                        len(extra_modifiers) == 0
                        and event.code not in self.MODIFIER_KEYS
                    ):
    ...
    def _check_shortcut_combination(self):
        ...
        if keys_match:
            ...
            if not self.combination_active and (
        ...
        if self.release_callback:
            try:
                ...
                callback_thread = threading.Thread(
                    target=self.release_callback, daemon=True
                )
                callback_thread.start()
            except Exception as e:
    ...
    def start(self) -> bool:
        ...
        if not self.devices:
            ...
            print(
                "[INFO] No keyboard access. Shortcut disabled; control recording via CLI (e.g. hyprwhspr record toggle) or recording_control file."
            )
        ...
        try:
            ...
            if self.grab_keys:
                if not self._setup_key_grabbing():
                    print(
                        "[ERROR] Failed to set up key grabbing - cannot start shortcuts"
                    )
                    return False
    ...
    def _start_hotplug_monitor(self):
        ...
        the service still works for already-connected keyboards. The user
        loses auto-detection for later hotplug events (the legacy behavior).
        """
        if not PYUDEV_AVAILABLE:
            print(
                "[HOTPLUG] pyudev not available; keyboards plugged in after "
    ...
    def _setup_key_grabbing(self) -> bool:
        ...
        try:
            ...
            for device in self.devices:
                for retry in range(
                    10
                ):  # Increased from 3 to 10 retries for better recovery
                    try:
                        device.grab()
                        grabbed_count += 1
                        break
                        ...
                        # Last retry failed - this is a real error
                        print(
                            f"[ERROR] Could not grab {device.name} after {retry + 1} retries: {e}"
                        )
                        print(
                            "[ERROR] Device may be in use by another process (e.g., Espanso, keyd, kmonad)"
                        )
                        print(
                            f"[ERROR] Check what's using it: sudo fuser {device.path}"
                        )
                        print(
                            f"[ERROR] If needed, kill the process: sudo fuser -k {device.path} (WARNING: kills all processes using this device)"
                        )
                        print(
                            "[ERROR] To avoid conflicts, use 'hyprwhspr keyboard list' to see available devices"
                        )
                        print(
                            "[ERROR] Then set 'selected_device_name' in config to use a different keyboard"
                        )
            ...
            if grabbed_count == 0:
                ...
                print(
                    "[ERROR]   1. Devices are in use by another tool (Espanso, keyd, kmonad, etc.)"
                )
                print("[ERROR]   2. Missing permissions (need to be in 'input' group)")
                print("[ERROR]   3. All devices are busy or inaccessible")
                print("[ERROR] Solutions:")
                print(
                    "[ERROR]   - Run 'hyprwhspr keyboard list' to see available devices"
                )
                print(
                    "[ERROR]   - Set 'selected_device_name' in config to use a different keyboard"
                )
                print(
                    "[ERROR]   - Add yourself to 'input' group: sudo usermod -aG input $USER"
                )
                print(
                    "[ERROR]   - Check for conflicting tools: sudo fuser /dev/input/event*"
                )
        ...
        try:
            ...
            if self.listener_thread and self.listener_thread.is_alive():
                ...
                if self.listener_thread.is_alive():
                    print(
                        "[WARN] Listener thread did not exit cleanly after 2 seconds, forcing cleanup"
                    )
    ...
    def is_active(self) -> bool:
        ...
        return (
            self.is_running and self.listener_thread and self.listener_thread.is_alive()
        )
...
def test_key_accessibility() -> Dict:
    ...
    results = {"accessible_devices": [], "inaccessible_devices": [], "total_devices": 0}
            ...
            if ecodes.EV_KEY in capabilities:
                try:
                    ...
                    results["accessible_devices"].append(
                        {"name": device.name, "path": device.path}
                    )
                except (OSError, IOError):
